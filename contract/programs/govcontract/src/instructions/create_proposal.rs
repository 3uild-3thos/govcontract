...
pub struct CreateProposal<'info> {
    ...
    #[account(
        constraint = vote_account.owner == &vote_program::ID,
        constraint = vote_account.data_len() >= VoteState::size_of()
    )]
...
impl<'info> CreateProposal<'info> {
    pub fn create_proposal(
        ...
        let vote_account_data = &self.vote_account.data.borrow();
        ...
        require_keys_eq!(node_pubkey, self.validator.key(), GovernanceError::InvalidVoteAccount);

        // Get cluster stake syscall 
        let cluster_stake = get_epoch_total_stake();
        msg!("Epoch stake {}", cluster_stake);
        // Get proposal creator stake
        let proposer_stake = get_epoch_stake_for_vote_account(self.validator.key);
        ...
        let proposer_stake_weight_bp = stake_weight_bp!(proposer_stake as u128, cluster_stake as u128)?;
